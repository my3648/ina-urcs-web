<template>
  <div class="login-container">
    ...
    <div class="out">
          <h3 style="color:#fff;text-align:center;font-size:22px;text-shadow: 0 0 3px #cac6c6;">UAES Calibration optimization and Simulation</h3>
  ...
  methods: {
    handleSubmit(event) {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          this.logining = true;
          this.$http.post("http://139.196.196.221:3011/userLogin", this.ruleForm).then(res => {
            console.log(res);
            // var { data, status } = res;
