...
<!-- 内部版 -->
  <div>
    ...
    <div v-if="configType=='build'" class="test" style="margin:0 auto;width:1100px;position:relative;">
      ...
      <!-- <el-dialog title="请选择类型" :close-on-click-modal="false" :visible.sync="platDialog" width="20%">
    
        <el-radio-group v-model="plat">
          <div style="margin:0.8em 0">
            <el-radio label="INA">联电</el-radio>
            <el-radio label="GWM">辉景</el-radio>
      ...
      <el-dialog title="请选择protoolType" :close-on-click-modal="false" :visible.sync="protoolDialog" width="20%">
        ...
        <el-radio-group @change="protoChange()" v-model="protoRadio">
          <div style="margin:0.8em 0" v-for="(item,index) in protocolArr" :key="item.protoolType">
          ...
          <div>
            <el-row :gutter="20">
              ...
              <el-col :span="18">
                ...
                <el-select size="small" style="width:120px;margin-right:0.5em" :disabled="baudDisabled"
                ...
                <el-table :data="tableDataSignal" tooltip-effect="dark" style="width: 100%">
                  ...
                  <el-table-column label="segment">
                    <template slot="header" slot-scope="scope">
                      ...
                      <span>{{protocol.CycleTime[0]||'segment'}}</span>
                    </template>
                  ...
                  <el-table-column label="10ms">
                    <template slot="header" slot-scope="scope">
                      ...
                      <span>{{protocol.CycleTime[1]||'10ms'}}</span>
                    </template>
                    <template slot-scope="scope">
                  ...
                  <el-table-column label="100ms">
                    <template slot="header" slot-scope="scope">
                      ...
                      <span>{{protocol.CycleTime[2]||'100ms'}}</span>
                    </template>
                    <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    submitA2l() {
      ...
      this.$http
        .post("http://47.101.67.237:4040/test/upload/a2lFile", formData, config)
        .then((res) => {
          console.log(res);
    ...
    analyA2l(data) {
      ...
      data.protocol.forEach((item, index) => {
        // item.CycleNum = [];
        var arr = [];
    ...
    protoChange() {
      this.protocol = this.protocolArr[this.protoRadio];
      this.baudRate = this.protocol.Baudrate;
      ...
      this.$http
        ...
        .then((res) => {
          ...
          if (res.data.res == "X") {
            ...
            console.log("A2L过滤后的LAB中的数据", data);
    ...
    labFunc(data, x, labItem) {
      ...
      this.lab_signal[labItem] = sigArr; //用于重新上传A2L时 ，重新选择暂存的LAB数据(移除了 待重启)
      console.log(labItem);
      console.log(this.lab_signal);
    },
    ...
    calculationPro(bk) {
      ...
      if (bk == "B_kl15") {
        signalArr[0] = signalArr[0].filter((value) => {
          return value.name != bk;
        });
        signalArr[1] = signalArr[1].filter((value) => {
          return value.name != bk;
        });
        signalArr[1].push({
          size: 1,
        ...
        signalArr[2] = signalArr[2].filter((value) => {
          return value.name != bk;
        });
      }
      if (this.fileType == "xcp") {
        ...
        var progressArr = get_load(this.fileType, this.protocol, signalArr);
    ...
    submitImg() {
      ...
      // }
      // DAQ = {
      //   "10ms time synchronous": { Measurement: [["a_b", "c_d"], []] },
      // };
      ...
      console.log(JSON.stringify(axiosData));
      ...
      //   console.log(a, b);
      // }
      // /test/generateJsonForRecord
      this.$http
        .post(
          "http://47.101.67.237:4040/test/upload/getsignalSELECT",
          axiosData
        )
        .then((res) => {
          console.log(res);
          if (res && res.status == 200 && res.data.res == "V") {
            ...
            // var row = res.data.data.recordConf[0];
            // this.jsonFile(row);
            this.sendJsonTable = [{ objName: res.data.objName }];
    ...
    submitBox(index) {
      ...
      var obj = {
        objName: this.sendJsonTable[0]["objName"],
        // uid: this.jsonuid,
        sn: index,
        // PA: this.username,
      ...
      this.$http
        .post("http://47.101.67.237:4040/test/upload/selectDevice", obj)
        .then((res) => {
          console.log(res);
          this.$message({
            type: "success",
